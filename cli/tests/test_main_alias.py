...
def test_rewrite_at_alias_for_agent_aware_subcommand() -> None:
    argv = ["expert", "@ecg", "ask", "hi", "--no-stream"]
...
def _seed(tmp_path: Path) -> Path:
    (tmp_path / "ecg").mkdir()
    (tmp_path / "derm").mkdir()
    (tmp_path / "ecg" / "agent_schema.yaml").write_text("x")
...
def test_which_with_agent_flag_overrides_pin(
    ...
    res = runner.invoke(app, ["which", "--agent", "ecg"])
    assert res.exit_code == 0, res.output
    assert "ecg" in res.output
